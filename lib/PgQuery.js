...
module.exports = class PgQuery {
  ...
  async init(uri) {
    ...
    checkType(params, 'connStrParams', 'string');
    // delay handling key, gzip, testOnStartup, prepareStatement, specInfo, and serverInfo

    if (params.connStrParams) {
      params.connStrParams = `?${params.connStrParams}`;
    } else {
      params.connStrParams = '';
    }
  ...
  static createPgPool(params) {
    const clientOpts = params.host.map((_, ind) => ({
      connectionString: `postgres://${params.username}:${params.password}@${params.host[ind]}:${params.port[ind]}/${params.database}${params.connStrParams}`,
