...
module.exports = class PgQuery {
  ...
  async init(uri) {
    ...
    checkType(params, 'prepareStatement', 'boolean');
    ...
    if (!('prepareStatement' in params)) {
      params.prepareStatement = prefer_prepared;
    }
    ...
    if (params.testOnStartup || !('testOnStartup' in params)) {
      const start = new Date().getTime();
      const test_zxy = [10, 513, 511];
      const result = await this.client.query(this.getTileQueryObj, test_zxy);
      console.log(`Test tile ${result} at ${test_zxy} was retrieved in: ${(new Date().getTime()) - start}`);
    }
  ...
  async _getTileAsync(z, x, y) {
    ...
    const res = await this.client.query(this.getTileQueryObj, [z, x, y]);
