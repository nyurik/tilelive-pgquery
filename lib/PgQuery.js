...
module.exports = class PgQuery {
  ...
  async init(uri) {
    ...
    // skip key, gzip, testOnStartup, prepareStatement, and serverInfo tests - handled later
    ...
    if (noGzip !== 'auto') {
      console.error('WARNING: parameter nogzip is now obsolete. Please use gzip parameter instead, but only if it cannot be auto-detected.');
