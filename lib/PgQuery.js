  ...
  async _getTileAsync(z, x, y, pool) {
    ...
    if (res.rows.length > 0) {
      ...
      if (row.length !== (this.useKeyColumn ? 2 : 1)) {
        throw new Error(`Expected ${this.useKeyColumn ? 'two' : 'one'} column, but got "${row.length}"`);
      }
      let value = row[0];
      if (value && value.length !== 0) {
        ...
        if (this.useKeyColumn) {
          value = await value;
          // some tilelive plugins like mbtiles understand key property, avoid recalculation
          value.key = row[1];
        }
        return value;
      }
  ...
  async printServerInfo(serverInfo) {
    if (serverInfo === undefined || toBool(serverInfo)) {
      ...
      const version_getter = async (pool) => {
        ...
        for (let setting of Object.keys(pgSettings)) {
          let value;
          try {
