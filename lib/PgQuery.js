...
module.exports = class PgQuery {
  ...
  async init(uri) {
    ...
    // skip testOnStartup, prepareStatement, and versionInfo tests - handled later
    ...
    await this.printVersions(params.versionInfo);
   ...
   */
  async printVersions(versionInfo) {
    if (versionInfo === undefined || toBool(versionInfo)) {
      const version_getter = async (pool) => {
        let info = `Version information for ${pool.pg.options.host}:${pool.pg.options.port}:\n`;
        `${pool.pg.options.host}:${pool.pg.options.port}`;
        for (let name of ['version', 'postgis_full_version']) {
          let value;
          try {
            const res = await pool.pg.query(`select ${name}() as v;`);
